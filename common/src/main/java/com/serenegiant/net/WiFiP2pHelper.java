...
public class WiFiP2pHelper {
	...
	public void connect(@NonNull final String remoteMacAddress) {
		if (DEBUG) {
            Log.v(TAG, "connect:remoteMacAddress=" + remoteMacAddress);
        }
	...
	public void connect(@NonNull final WifiP2pConfig config) throws IllegalStateException {
		if (DEBUG) {
            Log.v(TAG, "connect:config=" + config);
        }
		if (mChannel != null) {
	...
	private synchronized void updateDevice(final WifiP2pDevice device) {
		if (DEBUG) {
            Log.v(TAG, "updateDevice:device=" + device);
        }
...
// コールバックの呼び出し
	...
	protected void callOnStateChanged(final boolean enabled) {
		if (DEBUG) {
            Log.v(TAG, "callOnStateChanged:enabled=" + enabled);
        }
...
//================================================================================
	...
	private final WifiP2pManager.PeerListListener mPeerListListener
		...
		public void onPeersAvailable(final WifiP2pDeviceList peers) {
			if (DEBUG) {
                Log.v(TAG, "onPeersAvailable:peers=" + peers);
            }
	...
	private final WifiP2pManager.ConnectionInfoListener
		...
		public void onConnectionInfoAvailable(final WifiP2pInfo info) {
			if (DEBUG) {
                Log.v(TAG, "onConnectionInfoAvailable:info=" + info);
            }
			if (info != null) {
				callOnConnect(info);
			}
	...
	private static class WiFiDirectBroadcastReceiver extends BroadcastReceiver {
	    ...
	    public void onReceive(final Context context, final Intent intent) {
	        ...
	        if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action)) {
	        	if (DEBUG) {
                    Log.v(TAG, "onReceive:WIFI_P2P_STATE_CHANGED_ACTION");
                }
	        	// p2pの有効・無効状態が変化した時
	        	try {
					...
					if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED) {
						...
						if (DEBUG) {
                            Log.d(TAG, "P2P state changed, enabled");
                        }
					} else {
						mParent.setIsWifiP2pEnabled(false);
						mParent.resetData();
						if (DEBUG) {
                            Log.d(TAG, "P2P state changed, disabled");
                        }
					}
				} catch (final Exception e) {
				...
				if (DEBUG) {
                    Log.v(TAG, "onReceive:WIFI_P2P_PEERS_CHANGED_ACTION");
                }
	        	// p2pピアが変化した時
				...
				if (DEBUG) {
                    Log.v(TAG, "onReceive:WIFI_P2P_THIS_DEVICE_CHANGED_ACTION");
                }
	        	// 自デバイスの状態が変化した時
	        	try {
