...
public class SocketChannelDataLink extends AbstractChannelDataLink {
	...
	public Client connectTo(final String addr, final int port) throws IOException {
		if (DEBUG) {
            Log.v(TAG, "connectTo:addr=" + addr + ",port=" + port);
        }
		Client result;
		try {
	...
	public static class Client extends AbstractClient {
		...
		public Client(@NonNull final SocketChannelDataLink parent, final String addr, final int port) {
			super(parent, null);
			if (DEBUG) {
                Log.v(TAG, "Client#コンストラクタ:addr=" + addr + ",port=" + port);
            }
			mAddr = addr;
			mPort = port;
			internalStart();
        ...
        protected synchronized void init() throws IOException {
			...
			if (DEBUG) {
                Log.v(TAG, "Client#init:finished");
            }
		}
	...
	private class ServerTask implements Runnable {
		...
		if (DEBUG) {
			...
			if (DEBUG) {
                Log.v(TAG, "ServerTask#release:finished");
            }
		}
		...
		public void run() {
			...
			if (DEBUG) {
                Log.v(TAG, "ServerTask#run:finished");
            }
		}
		...
		private void serverLoop() {
			...
			if (DEBUG) {
                Log.v(TAG, "ServerTask#serverLoop:");
            }
			...
			for (; mIsRunning ; ) {
				...
				try {
					...
					if (DEBUG) {
                        Log.v(TAG, "ServerTask#serverLoop:ServerSocket#acceptでブロッキング");
                    }
					final SocketChannel clientChannel = mServerChannel.accept();
					if (DEBUG) {
                        Log.v(TAG, "ServerTask#serverLoop:ServerSocket#acceptを抜けた");
                    }
					final Client client = new Client(SocketChannelDataLink.this, clientChannel);
					add(client);
					if (DEBUG) {
                        Log.v(TAG, "ServerTask#serverLoop:クライアント開始");
                    }
				} catch (final IOException e) {
					break;
				}
			...
			if (DEBUG) {
                Log.v(TAG, "ServerTask#serverLoop:finished");
            }
		}
