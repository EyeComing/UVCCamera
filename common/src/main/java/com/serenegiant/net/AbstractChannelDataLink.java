...
public abstract class AbstractChannelDataLink {
	...
	public void add(final Callback callback) {
		if (DEBUG) {
            Log.v(TAG, "add:callback=" + callback);
        }
		if (callback != null) {
	...
	public void remove(final Callback callback) {
		if (DEBUG) {
            Log.v(TAG, "remove:callback=" + callback);
        }
	...
	public void remove(final AbstractClient client) {
		if (DEBUG) {
            Log.v(TAG, "remove:client=" + client);
        }
	...
	public static abstract class AbstractClient implements Runnable, Handler.Callback {
		...
		public AbstractClient(@NonNull final AbstractChannelDataLink parent,
			...
			if (DEBUG) {
                Log.v(TAG, "Client#コンストラクタ:channel=" + channel);
            }
		...
		protected void internalStart() {
			if (DEBUG) {
                Log.v(TAG, "Client#internalStart:");
            }
			synchronized (this) {
			...
			if (DEBUG) {
                Log.v(TAG, "Client#internalStart:finished");
            }
		}
		...
		private void internalRelease() {
			if (DEBUG) {
                Log.v(TAG, "Client#internalRelease:");
            }
			...
			if (DEBUG) {
                Log.v(TAG, "Client#internalRelease:finished");
            }
		}
		...
		private synchronized void send(final int type, @Nullable final Object msg) throws IOException {
			if (DEBUG) {
                Log.v(TAG, "Client#send:");
            }
			if ((mSenderHandler == null) || !mIsRunning || !mIsInit) {
		...
		private void doReceiveLoop()
			...
			for (; mIsRunning; ) {
				try {
					...
					if (DEBUG) {
                        Log.v(TAG, "Client#doReceiveLoop:type=" + type);
                    }
					switch (type) {
					case TYPE_NULL:
						callOnReceive(null);
...
//					if (DEBUG) Log.w(TAG, e);
			...
			if (DEBUG) {
                Log.v(TAG, "Client#doReceiveLoop:finished");
            }
		}
		
		protected void callOnConnect() {
			if (DEBUG) {
                Log.v(TAG, "callOnConnect:");
            }
			final AbstractChannelDataLink parent = mWeakParent.get();
			if (parent != null) {
		...
		protected void callOnReceive(@Nullable final Object msg) {
			if (DEBUG) {
                Log.v(TAG, "callOnReceive:msg=" + msg);
            }
			final AbstractChannelDataLink parent = mWeakParent.get();
			if (parent != null) {
				for (final Callback callback : parent.mCallbacks) {
