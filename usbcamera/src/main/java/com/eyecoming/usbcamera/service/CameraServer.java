    ...
    private static final class CameraThread extends Thread {
        ...
        public void handleOpen() {
            ...
            synchronized (mSync) {
                ...
                if (DEBUG) {
                    Log.i(TAG, "supportedSize:" + mUVCCamera.getSupportedSize());
                }
            }
        ...
        public void handleStartPreview(final int width, final int height, final Surface surface) {
            ...
            synchronized (mSync) {
                ...
                try {
                    mUVCCamera.setPreviewSize(width, height, 1, 61, UVCCamera.FRAME_FORMAT_MJPEG, UVCCamera.DEFAULT_BANDWIDTH);
                } catch (final IllegalArgumentException e) {
                    try {
                        // fallback to YUV mode
                        mUVCCamera.setPreviewSize(width, height, 1, 61, UVCCamera.DEFAULT_PREVIEW_MODE, UVCCamera.DEFAULT_BANDWIDTH);
...
//				new MediaSurfaceEncoder(mFrameWidth, mFrameHeight, mMuxer, mMediaEncoderListener);
        ...
        public void handleStopRecording() {
            if (DEBUG) {
                Log.d(TAG_THREAD, "handleStopRecording:mMuxer=" + mMuxer);
            }
            if (mMuxer != null) {
                synchronized (mSync) {
                    if (mUVCCamera != null) {
